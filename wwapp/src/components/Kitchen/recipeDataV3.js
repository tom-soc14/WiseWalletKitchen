// import spaghettiBol from "./img/Spaghetti_Bolognese.jpeg";
// import vegetableStir from "./img/Vegetable_Stir_Fry.jpeg";
// import tunaPasta from "./img/Tuna_Pasta.jpeg";
// import salmonSauce from "./img/Salmon_Sauce.jpeg";
// import mangoSalsa from "./img/mango_salsa.jpeg";
// import beanRice from "./img/beanRiceBurrito.jpeg";
// import chickenRice from "./img/chicken_rice.jpeg";

//DATA PLAN
// Get more data with the following credientials:
// Establish 3 different Recipe Types: Saver, Classic, Premium ✅
// Min - Max: Saver: (£3-£5), Classic: ❌
// // Done but not accurate for MVP
// + 7 days of each Recipe plan ✅
// + set a minimum and maximum cost per day for each plan ❌

// STRETCH GOALS
// Remove - HouseholdNo => per person quanitity/cost ✅
// + Add prices for individual items in the ingredients array PER PERSON ✅
// // Done but not accurate for MVP


let recipeDataV2 = [
    // Saver Recipes -----------------------------------------------------------------------
    {
      RecipeName: "Spaghetti Bolognese",
      RecipeID: "1001",
      RecipeType: "Saver",
      PrepTime: "15 minutes",
      CookingTime: "30 minutes",
      Price: [3.0, 4.5, 6.0, 7.5],
      Photo: "spaghettiBol",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Ground beef",
          IngredientId: "1",
          Amount: ["200g", "300g", "400g", "500g"],
          Price: [2.0, 3.0, 4.0, 5.0],
        },
        {
          Name: "Onion",
          IngredientId: "2",
          Amount: ["1", "1", "2", "2"],
          Price: [0.2, 0.2, 0.4, 0.4],
        },
      ],
      Instructions: [
        "1. Cook the ground beef in a pan until browned.",
        "2. Add the chopped onions and cook until softened.",
        "3. Pour in the tomato sauce and simmer for 20 minutes.",
        "4. Cook the spaghetti according to the package instructions.",
        "5. Serve the Bolognese sauce over the cooked spaghetti and enjoy.",
      ],
    },
    {
      RecipeName: "Chicken Stir-Fry",
      RecipeID: "1002",
      RecipeType: "Saver",
      PrepTime: "10 minutes",
      CookingTime: "15 minutes",
      Price: [4.0, 6.0, 8.0, 10.0],
      Photo: "vegetableStir",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "True",
      Ingredients: [
        {
          Name: "Chicken breast",
          IngredientId: "3",
          Amount: ["300g", "400g", "500g", "600g"],
          Price: [2.0, 3.0, 4.0, 5.0],
        },
        {
          Name: "Mixed vegetables",
          IngredientId: "4",
          Amount: ["200g", "300g", "400g", "500g"],
          Price: [2.0, 3.0, 4.0, 5.0],
        },
      ],
      Instructions: [
        "1. Heat oil in a wok or large skillet over high heat.",
        "2. Add the chicken and cook until browned.",
        "3. Add the mixed vegetables and stir-fry for a few minutes.",
        "4. Mix in the sauce and cook for another minute.",
        "5. Serve hot with steamed rice or noodles.",
      ],
    },
    {
      RecipeName: "Tuna Salad",
      RecipeID: "1003",
      RecipeType: "Saver",
      PrepTime: "10 minutes",
      CookingTime: "0 minutes",
      Price: [2.5, 3.5, 4.5, 5.5],
      Photo: "tunaPasta",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "True",
      Ingredients: [
        {
          Name: "Canned tuna",
          IngredientId: "5",
          Amount: ["1 can", "1 can", "1 can", "2 cans"],
          Price: [1.5, 2.5, 3.5, 4.5],
        },
        {
          Name: "Lettuce",
          IngredientId: "6",
          Amount: ["1 head", "1 head", "1 head", "2 heads"],
          Price: [1.0, 1.0, 2.0, 2.0],
        },
      ],
      Instructions: [
        "1. Drain the canned tuna and flake it into a bowl.",
        "2. Wash and tear the lettuce into bite-sized pieces.",
        "3. Mix the tuna and lettuce together in a large bowl.",
        "4. Add your favorite dressing and toss well.",
        "5. Serve immediately as a refreshing salad.",
      ],
    },
    {
      RecipeName: "Vegetable Curry",
      RecipeID: "1004",
      RecipeType: "Saver",
      PrepTime: "15 minutes",
      CookingTime: "25 minutes",
      Price: [4.5, 6.0, 8.0, 9.5],
      Photo: "vegetableCurry",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "True",
      GlutenFree: "True",
      Ingredients: [
        {
          Name: "Mixed vegetables",
          IngredientId: "7",
          Amount: ["500g", "600g", "700g", "800g"],
          Price: [2.5, 3.5, 4.5, 5.5],
        },
        {
          Name: "Coconut milk",
          IngredientId: "8",
          Amount: ["400ml", "400ml", "600ml", "600ml"],
          Price: [2.0, 2.5, 3.0, 3.5],
        },
      ],
      Instructions: [
        "1. Heat oil in a large pan over medium heat.",
        "2. Add the mixed vegetables and cook until tender.",
        "3. Pour in the coconut milk and bring to a simmer.",
        "4. Add the curry paste and stir well to combine.",
        "5. Simmer for 10 minutes, then serve with rice or naan bread.",
      ],
    },
    {
      RecipeName: "Egg Fried Rice",
      RecipeID: "1005",
      RecipeType: "Saver",
      PrepTime: "5 minutes",
      CookingTime: "10 minutes",
      Price: [3.0, 4.5, 5.5, 6.5],
      Photo: "eggFriedRice",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "True",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Cooked rice",
          IngredientId: "9",
          Amount: ["2 cups", "2 cups", "3 cups", "3 cups"],
          Price: [2.0, 3.0, 4.0, 5.0],
        },
        {
          Name: "Eggs",
          IngredientId: "10",
          Amount: ["2", "3", "3", "4"],
          Price: [1.0, 1.5, 2.0, 2.5],
        },
      ],
      Instructions: [
        "1. Heat oil in a large skillet or wok over medium heat.",
        "2. Beat the eggs in a small bowl and set aside.",
        "3. Add the cooked rice to the skillet and stir-fry for a few minutes.",
        "4. Push the rice to one side and pour the beaten eggs into the empty space.",
        "5. Scramble the eggs until cooked, then mix with the rice.",
        "6. Season with soy sauce or other seasonings of your choice.",
        "7. Serve hot as a main dish or side dish.",
      ],
    },
    {
      RecipeName: "Ham and Cheese Sandwich",
      RecipeID: "1006",
      RecipeType: "Saver",
      PrepTime: "5 minutes",
      CookingTime: "0 minutes",
      Price: [2.0, 2.5, 3.0, 3.5],
      Photo: "hamCheeseSandwich",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Ham slices",
          IngredientId: "11",
          Amount: ["4", "6", "6", "8"],
          Price: [1.5, 2.0, 2.5, 3.0],
        },
        {
          Name: "Cheese slices",
          IngredientId: "12",
          Amount: ["2", "3", "3", "4"],
          Price: [0.5, 0.5, 0.5, 0.5],
        },
        {
          Name: "Bread slices",
          IngredientId: "13",
          Amount: ["2", "3", "4", "4"],
          Price: [0.5, 1.0, 1.5, 2.0],
        },
      ],
      Instructions: [
        "1. Lay the bread slices on a clean surface.",
        "2. Place ham and cheese slices on one slice of bread.",
        "3. Top with the other bread slice to form a sandwich.",
        "4. Cut the sandwich in half if desired.",
        "5. Serve immediately or pack for a quick meal on the go.",
      ],
    },
    {
      RecipeName: "Pasta Primavera",
      RecipeID: "1007",
      RecipeType: "Saver",
      PrepTime: "10 minutes",
      CookingTime: "20 minutes",
      Price: [3.5, 4.5, 5.5, 6.5],
      Photo: "pastaPrimavera",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "True",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Pasta",
          IngredientId: "14",
          Amount: ["200g", "250g", "300g", "350g"],
          Price: [2.0, 2.5, 3.0, 3.5],
        },
        {
          Name: "Assorted vegetables",
          IngredientId: "15",
          Amount: ["2 cups", "3 cups", "3 cups", "4 cups"],
          Price: [1.5, 2.0, 2.5, 3.0],
        },
      ],
      Instructions: [
        "1. Cook the pasta according to the package instructions.",
        "2. In a separate pan, sauté the assorted vegetables until tender.",
        "3. Drain the cooked pasta and add it to the vegetables.",
        "4. Toss well and season with salt and pepper.",
        "5. Serve hot as a light and flavorful pasta dish.",
      ],
    },
    // Classic Recipes ---------------------------------------------------------------------------
    {
      RecipeName: "Beef Stroganoff",
      RecipeID: "2001",
      RecipeType: "Classic",
      PrepTime: "15 minutes",
      CookingTime: "45 minutes",
      Price: [8.0, 10.0, 12.0, 14.0],
      Photo: "beefStroganoff",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Beef tenderloin",
          IngredientId: "16",
          Amount: ["500g", "600g", "700g", "800g"],
          Price: [6.0, 7.5, 9.0, 10.5],
        },
        {
          Name: "Onion",
          IngredientId: "2",
          Amount: ["1", "1", "2", "2"],
          Price: [1.0, 1.0, 2.0, 2.0],
        },
        {
          Name: "Sour cream",
          IngredientId: "17",
          Amount: ["200g", "200g", "300g", "300g"],
          Price: [1.0, 1.5, 1.5, 2.0],
        },
      ],
      Instructions: [
        "1. Cut the beef tenderloin into thin strips.",
        "2. In a large pan, cook the beef until browned.",
        "3. Add the chopped onions and cook until softened.",
        "4. Stir in the sour cream and simmer for 30 minutes.",
        "5. Serve over cooked rice or egg noodles.",
      ],
    },
    {
      RecipeName: "Roast Chicken",
      RecipeID: "2002",
      RecipeType: "Classic",
      PrepTime: "10 minutes",
      CookingTime: "1 hour 30 minutes",
      Price: [12.0, 15.0, 18.0, 21.0],
      Photo: "roastChicken",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "True",
      Ingredients: [
        {
          Name: "Whole chicken",
          IngredientId: "18",
          Amount: ["1", "1", "1", "1"],
          Price: [10.0, 12.0, 14.0, 16.0],
        },
        {
          Name: "Potatoes",
          IngredientId: "19",
          Amount: ["4", "6", "8", "10"],
          Price: [2.0, 2.5, 3.0, 3.5],
        },
        {
          Name: "Carrots",
          IngredientId: "20",
          Amount: ["4", "6", "8", "10"],
          Price: [1.5, 2.0, 2.5, 3.0],
        },
      ],
      Instructions: [
        "1. Preheat the oven to 180°C (350°F).",
        "2. Place the whole chicken in a roasting pan.",
        "3. Season the chicken with salt, pepper, and herbs.",
        "4. Arrange the potatoes and carrots around the chicken.",
        "5. Roast in the oven for 1 hour and 30 minutes, or until the chicken is cooked through.",
        "6. Serve hot with the roasted vegetables.",
      ],
    },
    {
      RecipeName: "Lasagna",
      RecipeID: "2003",
      RecipeType: "Classic",
      PrepTime: "30 minutes",
      CookingTime: "1 hour",
      Price: [15.0, 18.0, 21.0, 24.0],
      Photo: "lasagna",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Ground beef",
          IngredientId: "1",
          Amount: ["500g", "600g", "700g", "800g"],
          Price: [6.0, 7.5, 9.0, 10.5],
        },
        {
          Name: "Lasagna noodles",
          IngredientId: "21",
          Amount: ["12", "16", "20", "24"],
          Price: [2.0, 3.0, 4.0, 5.0],
        },
        {
          Name: "Tomato sauce",
          IngredientId: "22",
          Amount: ["500g", "600g", "700g", "800g"],
          Price: [2.0, 2.5, 3.0, 3.5],
        },
      ],
      Instructions: [
        "1. Cook the ground beef in a pan until browned.",
        "2. Add the tomato sauce and simmer for 10 minutes.",
        "3. Cook the lasagna noodles according to the package instructions.",
        "4. In a baking dish, layer the noodles, meat sauce, and cheese.",
        "5. Repeat the layers until all ingredients are used.",
        "6. Bake in the oven at 180°C (350°F) for 45 minutes to 1 hour.",
        "7. Let it cool for a few minutes before serving.",
      ],
    },
    {
      RecipeName: "Beef Wellington",
      RecipeID: "2004",
      RecipeType: "Classic",
      PrepTime: "30 minutes",
      CookingTime: "45 minutes",
      Price: [20.0, 25.0, 30.0, 35.0],
      Photo: "beefWellington",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Beef fillet",
          IngredientId: "23",
          Amount: ["500g", "600g", "700g", "800g"],
          Price: [18.0, 21.0, 24.0, 27.0],
        },
        {
          Name: "Puff pastry",
          IngredientId: "24",
          Amount: ["1 sheet", "1 sheet", "2 sheets", "2 sheets"],
          Price: [2.0, 2.5, 4.0, 4.5],
        },
        {
          Name: "Mushrooms",
          IngredientId: "25",
          Amount: ["200g", "250g", "300g", "350g"],
          Price: [1.0, 1.5, 2.0, 2.5],
        },
      ],
      Instructions: [
        "1. Season the beef fillet with salt and pepper.",
        "2. Sear the beef in a hot pan until browned on all sides.",
        "3. In a separate pan, sauté the mushrooms until golden.",
        "4. Roll out the puff pastry and place the beef in the center.",
        "5. Top the beef with the sautéed mushrooms.",
        "6. Wrap the puff pastry around the beef, sealing the edges.",
        "7. Bake in the oven at 200°C (400°F) for 25-30 minutes.",
        "8. Let it rest for a few minutes before slicing.",
      ],
    },
    {
      RecipeName: "Chicken Parmesan",
      RecipeID: "2005",
      RecipeType: "Classic",
      PrepTime: "20 minutes",
      CookingTime: "25 minutes",
      Price: [14.0, 16.0, 18.0, 20.0],
      Photo: "chickenParmesan",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Chicken breasts",
          IngredientId: "26",
          Amount: ["4", "6", "6", "8"],
          Price: [8.0, 10.0, 10.0, 12.0],
        },
        {
          Name: "Breadcrumbs",
          IngredientId: "27",
          Amount: ["1 cup", "1 cup", "1 cup", "1 cup"],
          Price: [1.0, 1.0, 1.0, 1.0],
        },
        {
          Name: "Tomato sauce",
          IngredientId: "22",
          Amount: ["500g", "600g", "700g", "800g"],
          Price: [2.0, 2.5, 3.0, 3.5],
        },
        {
          Name: "Mozzarella cheese",
          IngredientId: "28",
          Amount: ["200g", "200g", "300g", "300g"],
          Price: [3.0, 3.0, 4.0, 4.0],
        },
      ],
      Instructions: [
        "1. Preheat the oven to 200°C (400°F).",
        "2. Flatten the chicken breasts to an even thickness.",
        "3. Coat the chicken with breadcrumbs.",
        "4. In a pan, cook the chicken until browned on both sides.",
        "5. Place the cooked chicken in a baking dish.",
        "6. Pour tomato sauce over the chicken and top with mozzarella cheese.",
        "7. Bake in the oven for 15-20 minutes, or until the cheese is melted and bubbly.",
        "8. Serve hot with spaghetti or garlic bread.",
      ],
    },
    {
      RecipeName: "Chicken Marsala",
      RecipeID: "2002",
      RecipeType: "Classic",
      PrepTime: "10 minutes",
      CookingTime: "30 minutes",
      Price: [10.0, 12.0, 14.0, 16.0],
      Photo: "chickenMarsala",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "True",
      Ingredients: [
        {
          Name: "Chicken breasts",
          IngredientId: "18",
          Amount: ["4", "4", "6", "6"],
          Price: [8.0, 10.0, 12.0, 14.0],
        },
        {
          Name: "Mushrooms",
          IngredientId: "19",
          Amount: ["200g", "200g", "300g", "300g"],
          Price: [2.0, 2.0, 3.0, 3.0],
        },
        {
          Name: "Marsala wine",
          IngredientId: "20",
          Amount: ["1 cup", "1 cup", "1 1/2 cups", "1 1/2 cups"],
          Price: [4.0, 4.0, 6.0, 6.0],
        },
      ],
      Instructions: [
        "1. Season the chicken breasts with salt and pepper.",
        "2. In a large skillet, cook the chicken until browned and cooked through. Remove from skillet and set aside.",
        "3. In the same skillet, add the mushrooms and cook until tender.",
        "4. Pour in the Marsala wine and simmer for a few minutes.",
        "5. Return the chicken to the skillet and simmer for another 5 minutes.",
        "6. Serve with mashed potatoes or pasta.",
      ],
    },
    {
      RecipeName: "Creamy Garlic Shrimp Pasta",
      RecipeID: "2003",
      RecipeType: "Seafood",
      PrepTime: "10 minutes",
      CookingTime: "20 minutes",
      Price: [12.0, 14.0, 16.0, 18.0],
      Photo: "garlicShrimpPasta",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Shrimp",
          IngredientId: "21",
          Amount: ["500g", "600g", "700g", "800g"],
          Price: [10.0, 12.0, 14.0, 16.0],
        },
        {
          Name: "Garlic",
          IngredientId: "22",
          Amount: ["4 cloves", "4 cloves", "6 cloves", "6 cloves"],
          Price: [1.0, 1.0, 1.5, 1.5],
        },
        {
          Name: "Heavy cream",
          IngredientId: "23",
          Amount: ["1 cup", "1 cup", "1 1/2 cups", "1 1/2 cups"],
          Price: [3.0, 3.0, 4.5, 4.5],
        },
        {
          Name: "Parmesan cheese",
          IngredientId: "24",
          Amount: ["1/2 cup", "1/2 cup", "3/4 cup", "3/4 cup"],
          Price: [1.0, 1.0, 1.5, 1.5],
        },
      ],
      Instructions: [
        "1. Cook the pasta according to the package instructions. Drain and set aside.",
        "2. In a large skillet, heat olive oil and add the minced garlic. Cook until fragrant.",
        "3. Add the shrimp to the skillet and cook until pink and cooked through.",
        "4. Reduce the heat and pour in the heavy cream. Simmer for a few minutes.",
        "5. Stir in the Parmesan cheese until melted and creamy.",
        "6. Add the cooked pasta to the skillet and toss to coat in the sauce.",
        "7. Serve with a sprinkle of chopped parsley.",
      ],
    },
  
  
    // Exclusive Recipes ---------------------------------------------------------------------
    {
      RecipeName: "Grilled Salmon",
      RecipeID: "3001",
      RecipeType: "Exclusive",
      delete: "2",
      PrepTime: "10 minutes",
      CookingTime: "15 minutes",
      Price: [18.0, 20.0, 22.0, 24.0],
      Photo: "grilledSalmon",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "True",
      Ingredients: [
        {
          Name: "Salmon fillets",
          IngredientId: "29",
          Amount: ["2", "2", "3", "3"],
          Price: [14.0, 16.0, 16.0, 18.0],
        },
        {
          Name: "Lemon",
          IngredientId: "30",
          Amount: ["1", "1", "1", "1"],
          Price: [1.0, 1.0, 1.0, 1.0],
        },
        {
          Name: "Fresh dill",
          IngredientId: "31",
          Amount: [
            "1 tablespoon",
            "1 tablespoon",
            "2 tablespoons",
            "2 tablespoons",
          ],
          Price: [3.0, 3.0, 3.0, 3.0],
        },
      ],
      Instructions: [
        "1. Preheat the grill to medium-high heat.",
        "2. Season the salmon fillets with salt and pepper.",
        "3. Place the salmon on the grill, skin-side down.",
        "4. Cook for 4-5 minutes on each side, or until the salmon is cooked through.",
        "5. Squeeze fresh lemon juice over the cooked salmon.",
        "6. Sprinkle with chopped dill.",
        "7. Serve hot with a side of steamed vegetables or a salad.",
      ],
    },
    {  
    // PRICE CHANGED Below - Nick and Tom - 19/06/23
    RecipeName: "Lobster Thermidor",
    RecipeID: "3002",
    RecipeType: "Exclusive",
    PrepTime: "20 minutes",
    CookingTime: "30 minutes",
    Price: [20.0, 25.0, 30.0, 35.0],
    Photo: "lobsterThermidor",
    PhotoAlt: "Description of PHOTOURL",
    Vegetarian: "False",
    GlutenFree: "True",
    Ingredients: [
      {
        Name: "Lobster tails",
        IngredientId: "32",
        Amount: ["2", "2", "2", "2"],
        Price: [5.0, 5.0, 5.0, 5.0],
      },
      {
        Name: "Butter",
        IngredientId: "33",
        Amount: ["4 tablespoons", "4 tablespoons", "6 tablespoons", "6 tablespoons"],
        Price: [4.0, 4.0, 6.0, 6.0],
      },
      {
        Name: "White wine",
        IngredientId: "34",
        Amount: ["1/4 cup", "1/4 cup", "1/2 cup", "1/2 cup"],
        Price: [5.0, 6.0, 7.0, 8.0],
      },
      {
        Name: "Dijon mustard",
        IngredientId: "35",
        Amount: ["1 tablespoon", "1 tablespoon", "2 tablespoons", "2 tablespoons"],
        Price: [6.0, 10.0, 14.0, 17.0],
      },
    ],
      Instructions: [
        "1. Preheat the oven to 200°C (400°F).",
        "2. Cut the lobster tails in half lengthwise.",
        "3. In a pan, melt the butter over medium heat.",
        "4. Add the lobster tails and cook until the meat is opaque.",
        "5. Remove the lobster meat from the shells and set aside.",
        "6. In the same pan, add white wine and dijon mustard.",
        "7. Cook until the sauce is thickened.",
        "8. Place the lobster meat back in the shells and spoon the sauce over the top.",
        "9. Bake in the oven for 15 minutes, or until the lobster is heated through and golden on top.",
        "10. Serve hot as a luxurious seafood dish.",
      ],
    },
  {
      // PRICE + RECIPE CHANGED Below - Nick and Tom - 19/06/23
      RecipeName: "Truffle Risotto",
      RecipeID: "3003",
      RecipeType: "Exclusive",
      PrepTime: "10 minutes",
      CookingTime: "30 minutes",
      Price: [15.0, 16.0, 17.0, 18.0],
      Photo: "truffleRisotto",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "True",
      GlutenFree: "True",
      Ingredients: [
        {
          Name: "Arborio rice",
          IngredientId: "36",
          Amount: ["1 cup", "1 cup", "2 cups", "2 cups"],
          Price: [10.0, 10.0, 20.0, 20.0],
        },
        {
          Name: "Vegetable broth",
          IngredientId: "37",
          Amount: ["4 cups", "4 cups", "6 cups", "6 cups"],
          Price: [4.0, 4.0, 6.0, 6.0],
        },
        {
          Name: "Truffle oil",
          IngredientId: "38",
          Amount: ["2 tablespoons", "2 tablespoons", "3 tablespoons", "3 tablespoons"],
          Price: [1.0, 2.0, 3.0, 4.0],
        },
      ],
      Instructions: [
        "1. In a large saucepan, heat the vegetable broth until simmering.",
        "2. In a separate pan, heat truffle oil over medium heat.",
        "3. Add the Arborio rice to the pan and cook for 2-3 minutes, stirring constantly.",
        "4. Add a ladleful of hot vegetable broth to the rice and stir until the liquid is absorbed.",
        "5. Repeat the process, adding more broth and stirring until the rice is cooked al dente.",
        "6. The risotto should be creamy but still have a slight bite.",
        "7. Remove from heat and let it rest for a minute.",
        "8. Serve the truffle risotto hot, drizzled with additional truffle oil if desired.",
      ],
    },
  {
      // PRICE + RECIPE CHANGED Below - Nick and Tom - 19/06/23
      RecipeName: "Creamy Garlic Parmesan Chicken",
      RecipeID: "3005",
      RecipeType: "Exclusive",
      PrepTime: "10 minutes",
      CookingTime: "25 minutes",
      Price: [10.0, 12.0, 14.0, 16.0],
      Photo: "creamyChicken",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "True",
      Ingredients: [
        {
          Name: "Chicken breasts",
          IngredientId: "43",
          Amount: ["4", "4", "6", "6"],
          Price: [6.0, 6.0, 9.0, 9.0],
        },
        {
          Name: "Garlic cloves",
          IngredientId: "44",
          Amount: ["4", "4", "6", "6"],
          Price: [1.0, 1.0, 1.5, 1.5],
        },
        {
          Name: "Parmesan cheese",
          IngredientId: "45",
          Amount: ["1/2 cup", "1/2 cup", "3/4 cup", "3/4 cup"],
          Price: [2.0, 2.0, 3.0, 3.0],
        },
        {
          Name: "Heavy cream",
          IngredientId: "46",
          Amount: ["1/2 cup", "1/2 cup", "3/4 cup", "3/4 cup"],
          Price: [1.0, 1.0, 1.5, 1.5],
        },
        {
          Name: "Butter",
          IngredientId: "47",
          Amount: ["2 tablespoons", "2 tablespoons", "3 tablespoons", "3 tablespoons"],
          Price: [0.5, 0.5, 1.0, 1.0],
        },
        {
          Name: "Fresh parsley",
          IngredientId: "48",
          Amount: ["1 tablespoon", "1 tablespoon", "2 tablespoons", "2 tablespoons"],
          Price: [0.5, 0.5, 1.0, 1.0],
        },
      ],
      Instructions: [
        "1. Season the chicken breasts with salt and black pepper on both sides.",
        "2. Heat butter in a large skillet over medium-high heat.",
        "3. Cook the chicken breasts for 6-8 minutes per side, or until cooked through.",
        "4. Remove the chicken from the skillet and set aside.",
        "5. In the same skillet, add minced garlic and cook for 1 minute until fragrant.",
        "6. Pour in the heavy cream and bring to a simmer.",
        "7. Stir in grated Parmesan cheese until melted and smooth.",
        "8. Return the chicken breasts to the skillet and simmer for 2-3 minutes, until the sauce thickens.",
        "9. Sprinkle with fresh parsley and serve hot.",
      ],},
    
    {
      // PRICE CHANGED Below - Nick and Tom - 19/06/23
      RecipeName: "Beef Wellington with Truffle",
      RecipeID: "3005",
      RecipeType: "Exclusive",
      PrepTime: "30 minutes",
      CookingTime: "45 minutes",
      Price: [22.0, 26.0, 31.0, 35.0],
      Photo: "beefWellingtonTruffle",
      PhotoAlt: "Description of PHOTOURL",
      Vegetarian: "False",
      GlutenFree: "False",
      Ingredients: [
        {
          Name: "Beef tenderloin",
          IngredientId: "43",
          Amount: ["1.5 kg", "1.5 kg", "2 kg", "2 kg"],
          Price: [10.0, 12.0, 14.0, 16.0],
        },
        {
          Name: "Puff pastry",
          IngredientId: "24",
          Amount: ["2 sheets", "2 sheets", "3 sheets", "3 sheets"],
          Price: [4.0, 4.0, 6.0, 6.0],
        },
        {
          Name: "Mushrooms",
          IngredientId: "25",
          Amount: ["300g", "300g", "400g", "400g"],
          Price: [2.0, 2.0, 3.0, 3.0],
        },
        {
          Name: "Truffle paste",
          IngredientId: "44",
          Amount: [
            "2 tablespoons",
            "2 tablespoons",
            "3 tablespoons",
            "3 tablespoons",
          ],
          Price: [6.0, 8.0, 8.0, 10.0],
        },
      ],
      Instructions: [
        "1. Season the beef tenderloin with salt and pepper.",
        "2. Sear the beef in a hot pan until browned on all sides.",
        "3. In a separate pan, sauté the mushrooms until golden.",
        "4. Roll out the puff pastry and spread truffle paste on it.",
        "5. Place the seared beef on top of the truffle paste.",
        "6. Spread the sautéed mushrooms over the beef.",
        "7. Wrap the puff pastry around the beef, sealing the edges.",
        "8. Bake in the oven at 200°C (400°F) for 35-40 minutes, or until the pastry is golden brown.",
        "9. Let the beef wellington rest for a few minutes before slicing.",
        "10. Serve hot as a luxurious and flavorful main dish.",
      ],
    },
    {
      // PRICE CHANGED Below - Nick and Tom - 19/06/23
        RecipeName: "Stuffed Portobello Mushrooms",
        RecipeID: "3002",
        RecipeType: "Exclusive",
        delete: "2",
        PrepTime: "20 minutes",
        CookingTime: "25 minutes",
        Price: [8.0, 10.0, 12.0, 14.0],
        Photo: "stuffedMushrooms",
        PhotoAlt: "Description of PHOTOURL",
        Vegetarian: "True",
        GlutenFree: "True",
        Ingredients: [
          {
            Name: "Portobello mushrooms",
            IngredientId: "32",
            Amount: ["4", "4", "6", "6"],
            Price: [2.0, 2.0, 3.0, 3.0],
          },
          {
            Name: "Spinach",
            IngredientId: "33",
            Amount: ["2 cups", "2 cups", "3 cups", "3 cups"],
            Price: [1.0, 1.0, 1.5, 1.5],
          },
          {
            Name: "Feta cheese",
            IngredientId: "34",
            Amount: ["1/2 cup", "1/2 cup", "3/4 cup", "3/4 cup"],
            Price: [3.0, 3.0, 4.5, 4.5],
          },
          {
            Name: "Cherry tomatoes",
            IngredientId: "35",
            Amount: ["1/2 cup", "1/2 cup", "3/4 cup", "3/4 cup"],
            Price: [1.0, 1.0, 1.5, 1.5],
          },
          {
            Name: "Balsamic glaze",
            IngredientId: "36",
            Amount: ["2 tablespoons", "2 tablespoons", "3 tablespoons", "3 tablespoons"],
            Price: [1.0, 1.0, 1.5, 1.5],
          },
        ],
      Instructions: [
        "1. Preheat the oven to 375°F (190°C).",
        "2. Clean the portobello mushrooms and remove the stems.",
        "3. In a skillet, sauté the spinach until wilted.",
        "4. In a bowl, combine the sautéed spinach, crumbled feta cheese, and halved cherry tomatoes.",
        "5. Place the mushroom caps on a baking sheet and fill them with the spinach mixture.",
        "6. Drizzle with balsamic glaze.",
        "7. Bake for 20-25 minutes, or until the mushrooms are tender and the filling is heated through.",
        "8. Serve as an appetizer or a main dish with a side of mixed greens.",
      ],
    },
    {
      // PRICE CHANGED Below - Nick and Tom - 19/06/23
        RecipeName: "Quinoa-Stuffed Bell Peppers",
        RecipeID: "3004",
        RecipeType: "Exclusive",
        delete: "2",
        PrepTime: "25 minutes",
        CookingTime: "35 minutes",
        Price: [10.0, 12.0, 14.0, 16.0],
        Photo: "quinoaStuffedPeppers",
        PhotoAlt: "Description of PHOTOURL",
        Vegetarian: "True",
        GlutenFree: "True",
        Ingredients: [
          {
            Name: "Bell peppers",
            IngredientId: "39",
            Amount: ["4", "4", "6", "6"],
            Price: [2.0, 2.0, 3.0, 3.0],
          },
          {
            Name: "Quinoa",
            IngredientId: "40",
            Amount: ["1 cup", "1 cup", "1 1/2 cups", "1 1/2 cups"],
            Price: [3.0, 3.0, 4.0, 4.0],
          },
          {
            Name: "Red onion",
            IngredientId: "41",
            Amount: ["1/2 cup", "1/2 cup", "3/4 cup", "3/4 cup"],
            Price: [2.0, 2.0, 2.5, 2.5],
          },
          {
            Name: "Zucchini",
            IngredientId: "42",
            Amount: ["1/2 cup", "1/2 cup", "3/4 cup", "3/4 cup"],
            Price: [1.0, 1.0, 1.5, 1.5],
          },
          {
            Name: "Goat cheese",
            IngredientId: "43",
            Amount: ["1/2 cup", "1/2 cup", "3/4 cup", "3/4 cup"],
            Price: [1.0, 1.0, 2.0, 2.0],
          },
          {
            Name: "Fresh basil",
            IngredientId: "44",
            Amount: ["2 tablespoons", "2 tablespoons", "3 tablespoons", "3 tablespoons"],
            Price: [1.0, 1.0, 1.5, 1.5],
          },
        ],
      
      
      Instructions: [
        "1. Preheat the oven to 375°F (190°C).",
        "2. Cut the tops off the bell peppers and remove the seeds and membranes.",
        "3. Cook the quinoa according to the package instructions.",
        "4. In a skillet, sauté the red onion and zucchini until softened.",
        "5. In a bowl, combine the cooked quinoa, sautéed vegetables, crumbled goat cheese, and chopped fresh basil.",
        "6. Stuff the mixture into the bell peppers.",
        "7. Place the stuffed peppers on a baking sheet and bake for 30-35 minutes, or until the peppers are tender.",
        "8. Serve hot as a main dish or a side.",
      ],
    }
  
  ];
  
  export default recipeDataV2;
  